...
float altOffset = 0.0;
bool init = false;
const uint16_t NOOFSAMPLES = 10;
uint32_t samplesCnt = 0;
uint32_t medSamplesCnt = 0;

float get_percent_error(float a, float b) {
    float tmp = a - b;
    if(tmp < 0){
        tmp = tmp * -1;
    }
    float ret = (tmp / a) * 100; // to percent
    return ret;
}
...
int main() {
    ...
    float altArr[NOOFSAMPLES];
    memset(altArr, 0, sizeof altArr);

    float filteredAlt = 0.0;

    float altArrMed[NOOFSAMPLES];
    memset(altArrMed, 0, sizeof altArrMed);
    float medFilteredAlt = 0.0;
    float prevAltValue = 0.0;
    ...
    while (1) {
        ...
        // filter
        altArr[++samplesCnt % NOOFSAMPLES] = altitude;

        filteredAlt = 0.0;
        for(int i = 0; i < NOOFSAMPLES; ++i) {
            filteredAlt = filteredAlt + altArr[i];
        }
        filteredAlt = filteredAlt / NOOFSAMPLES;

        printf("Filtered alt. = %.2f m\n", filteredAlt);


        // median filter
        if(samplesCnt % NOOFSAMPLES > 0) {
            prevAltValue = altArr[(samplesCnt % NOOFSAMPLES) - 1];
        }

        printf("prevAltValue = %.2f, altitude = %.2f\n", prevAltValue, altitude);
        printf("get_percent_error = %.2f, %s\n", get_percent_error(prevAltValue, altitude), get_percent_error(prevAltValue, altitude) < 10 ? "GOOD": "BAD");
        if(get_percent_error(prevAltValue, altitude) < 20) {
            altArrMed[++medSamplesCnt % NOOFSAMPLES] = altitude;
        }
        medFilteredAlt = 0.0;
        for(int i = 0; i < NOOFSAMPLES; ++i) {
            medFilteredAlt = medFilteredAlt + altArrMed[i];
        }
        medFilteredAlt = medFilteredAlt / NOOFSAMPLES;

        printf("Median filtered alt. = %.2f m\n", medFilteredAlt);
